...
function checkNumber(keys, index) {
  return every(keys, (value, header) => {
    if (isNaN(value)) {
      logger.log(1, "datum", "not a number", {
        line: index + 1,
        value: value,
        header: header,
      });
...
export function parseSpanValueData(data, layoutSummary) {
  ...
  const filteredData = data.filter((datum, index) =>
  ...
  const filteredData = data
    ...
    .filter((datum) => {
      if (datum.start < 0 || datum.end > layoutSummary[datum.block_id]) {
        logger.log(2, "position", "position inconsistency", {
          datum: datum,
          layoutSummary: layoutSummary,
        });
        return false;
...
export function parsePositionTextData(data, layoutSummary) {
  ...
  const filteredData = data
    ...
    .filter((datum, index) => checkNumber({ position: datum.position }, index));
